...
class PreTrainedModel(nn.Module, ModuleUtilsMixin):
    ...
    def generate(
        ...
        Parameters:
            ...
            For blending of topics we added these parameters:
            only_get_gedi_probs: (`optional`) bool
                If True, this generation function will instead return a number representing confidence of sentence generated by input topic. Supports single topics only.

            blending_params: (`optional`) float
                (Default 1) Changes how weights are treated in the blending process.
                The larger this value, the more topics are blended equally.
    ...
    def _generate_no_beam_search(
        ...
        # This method is modified to support multiple control codes.

        # Determine whether right padding modifications need to be applied.
        adjust_for_right_paddings = False
        ...
        crossentropy_loss_weight = torch.tensor(crossentropy_loss_weight).float().to(input_ids.device)

        # Parsing multicode if it's in new format.
        multi_code_item = []
        multi_code_weights = []
        if type(multi_code) is dict:
            for key in multi_code:
                multi_code_item.append(key)
                multi_code_weights.append(multi_code[key])
        elif type(multi_code) is list:
            multi_code_item = multi_code
            multi_code_weights = [1]*len(multi_code)

        # normalize it so they sum to 1
        sum_of_weights = sum(multi_code_weights)
        for idx in range(len(multi_code_weights)):
            multi_code_weights[idx] /= sum_of_weights

        # How many copies do we need in the batch?
        control_code_count = len(multi_code)
        total_copies_count = control_code_count * 2
        ...
        if not (gedi_model is None):
            ...
            # list for all sequences to be processed.
            all_seqs = []

            if not (multi_code_item is None):
                #seq_a2 = torch.LongTensor(multi_code).unsqueeze(0).to(seq_a.device)
                for item in multi_code_item:
                    #seq_a2 = torch.LongTensor([item]).unsqueeze(0).to(seq_a.device)
                    seq_a2 = (torch.ones(input_ids.shape[0]) * item).type_as(input_ids).view(-1, 1)
                    seq_a_new = torch.cat((seq_a, seq_a2, input_ids), dim=1)[:, :]
                    seq_b_new = torch.cat((seq_b, seq_a2, input_ids), dim=1)[:, :]
                    all_seqs.append(seq_a_new)
                    all_seqs.append(seq_b_new)

            else:
                seq_a = torch.cat((seq_a, input_ids.view(1,-1)), dim=1)[:, :]
                seq_b = torch.cat((seq_b, input_ids.view(1,-1)), dim=1)[:, :]
                all_seqs.append(seq_a)
                all_seqs.append(seq_b)

            #bsz = seq_a.shape[0]

            # modified: make total_copies_count items here
            seq_batched = torch.cat(all_seqs, dim=0)
            if pad_lens is None:

                gedi_pad_lens = None
            else:
                gedi_pad_lens = pad_lens * total_copies_count #pad_lens + pad_lens
        ...
        while cur_len < max_length:
            ...
            masks = (input_ids != pad_token_id).type(torch.LongTensor).to(input_ids.device)
            if adjust_for_right_paddings:
                model_inputs["attention_mask"] = masks # padding token used
            # prompt length. This assumes prompt starts from the beginning and does not break.
            ...
            if not (gpt3_api_key is None):
                ...
                if adjust_for_right_paddings and iteration_count == 0:
                    # We need to ensure padded tokens are not in considerations here.
                    last_effective_index = prompt_length - 1
                    next_token_logits = []
                    for index, item in enumerate(last_effective_index):
                        # for each batch input sequence
                        next_token_logits.append(outputs[0][index,item,:])
            ...
            if not (gedi_model is None):
                ...
                if not gedi_past is None:
                    input_batched = model_inputs["input_ids"].repeat(total_copies_count,1)
                ...
                gedi_mask = (seq_batched != pad_token_id).type(torch.IntTensor)
                if adjust_for_right_paddings:
                    inputs["attention_mask"] = gedi_mask # padding token used
                prompt_length = torch.sum(gedi_mask, dim=-1)
                # print("inputs = %s" % seq_batched)
                # print("masks = %s" % gedi_mask)
                gedi_outputs = gedi_model(**inputs)
                # print("-----")
                # #print(inputs['attention_mask'])
                # print(gedi_outputs[0][0][-1][100])
                # print("-----")

                logp_all = []
                if gedi_past is None:
                    # Since gedi_past is None, the model hasn't inferred anything.
                    # We should take actual logits from places not masked.
                    ...
                    if gedi_outputs[0].shape[1] > 1:
                        ...
                        if adjust_for_right_paddings:
                            # changed: because we are doing batching...
                            assert seq_len == int(prompt_length.max() - 1) # Because the longest sequence minus 1 for shifting logits should have seq_len as length
                            logits_r_split = torch.split(logits_r / (prompt_length.to(input_ids.device) - 1), input_ids.shape[0])
                        else:
                            logits_r_split = torch.split(logits_r / seq_len, input_ids.shape[0])

                        logits0 = torch.stack(logits_r_split,1)
                        ...
                        logp_softmax = torch.log_softmax(logits0, -1)
                        for i in range(total_copies_count): # This should be exactly the same as shape[1]
                            logp_all.append(logp_softmax[:,i])


                        #logp_desired = torch.log_softmax(logits0, -1)[:, 0]
                        #logp_undesired = torch.log_softmax(logits0, -1)[:, 1]

                        prob_fn = lambda x: torch.exp(torch.log_softmax(logits0, -1)[:, x][0]).item()

                        # p_desired = torch.exp(logp_desired[0])
                        # p_undesired = torch.exp(logp_undesired[0])
                        if only_get_gedi_probs:
                            all_probs_to_return = {}
                            for index in range(control_code_count):
                                result = prob_fn(index * 2)
                                all_probs_to_return[multi_code_item[index]] = result
                                # print("GeDi estimates the probability that the input sentence is desired class %s is: "%index +
                                #       str(result)
                                #       )
                            return all_probs_to_return
                    else:
                        seq_len = 0
                        logp_desired = (torch.zeros(input_ids.shape[0]) + torch.log(torch.tensor(0.5))).to(
                            input_ids.device)
                        logp_undesired = (torch.zeros(input_ids.shape[0]) + torch.log(torch.tensor(0.5))).to(
                            input_ids.device)
                ...
                if iteration_count == 0 and adjust_for_right_paddings:
                    last_effective_index = prompt_length - 1
                    gedi_logits = []
                    for index, item in enumerate(last_effective_index):
                        # for each batch input sequence
                        gedi_logits.append(gedi_outputs[0][index,item,:])
                    gedi_logits = torch.stack(gedi_logits)
                    #print("Gedi Logits shape = %s"%str(gedi_logits.shape))
                    gedi_logits = torch.log_softmax(gedi_logits,-1)
                    #print("Gedi Logits 100: %s"%gedi_logits[0][100])
                else:
                    gedi_logits = torch.log_softmax(gedi_outputs[0][:, -1, :], -1)
                    #print("Gedi Logits 100*: %s" % gedi_logits[0][100])


                gedi_logits += logits_r.unsqueeze(1)

                #print("SEQ_LEN = %s, arr = %s"%(seq_len,prompt_length))
                if adjust_for_right_paddings:
                    gedi_logits_split = torch.split(gedi_logits / (prompt_length.expand(gedi_logits.shape[1],-1).transpose(0,1).to(input_ids.device)), input_ids.shape[0])
                else:
                    gedi_logits_split = torch.split(gedi_logits / seq_len,
                                                    input_ids.shape[0])

                # logits_pos = gedi_logits_split[0]
                # logits_neg = gedi_logits_split[1]
                ...
                # # logp_..._t is log_softmax of logits
                # logp_desired_t = torch.log_softmax(logits, -1)[:, :, 0]
                # logp_undesired_t = torch.log_softmax(logits, -1)[:, :, 1]

                logp_t = []
                if total_copies_count % 2 != 0:
                    raise RuntimeError("total_copies_count is not divisible by 2. Should never happen.")
                for i in range(int(total_copies_count / 2)):
                    logp_related = torch.stack((logits[:,:,2*i],logits[:,:,2*i+1]),2)
                    logp_related_softmax = torch.log_softmax(logp_related,-1)
                    logp_t.append(logp_related_softmax[:, :, 0])
                    logp_t.append(logp_related_softmax[:, :, 1])

                # next_token_logits = torch.log_softmax(1 * next_token_logits, -1) + disc_weight * (
                #     logp_desired_t)  # +delta_capped82058721

                #print("NX_TOKEN_LOG 0 100 = %s"%next_token_logits[0][100])
                next_token_logits = torch.log_softmax(1 * next_token_logits, -1)
                count = -1
                for index,item in enumerate(logp_t):
                    if index % 2 == 0:
                        desired = True
                        count += 1
                    else:
                        desired = False
                    if desired:
                        #contrast_item = logp_t[index + 1] # item for contrasted probabilities
                        # Because we have 1 for desired and 1 for undesired
                        normalization_sum = sum(pow(x,blending_param) for x in multi_code_weights)
                        if normalization_sum < 0.00001:
                            raise RuntimeWarning("Normalization_sum too small: Topic weights wrongly set?")
                        normalization_factor = 1 / normalization_sum
                        next_token_logits += disc_weight * normalization_factor * pow(multi_code_weights[count],blending_param) * (item)

                # filtering. TODO: fix it
            ...
            #print("gedi_output_0 = %s"%str(gedi_outputs[0][0][0][100]))
            #print("gedi_output_1 = %s"%str(gedi_outputs[1][0][0][0][0][-1][10]))
            if not (gedi_model is None):
            ...
            if no_repeat_ngram_size > 0:
                ...
                banned_tokens = calc_banned_ngram_tokens(input_ids[cond_len:], batch_size, no_repeat_ngram_size,
                                                         len(input_ids[cond_len:]))
            ...
            if not (gedi_model is None):
                token_list = next_token.tolist() * total_copies_count
        ...
        # if not (gedi_model is None):
        #     try:
        #         print("GeDi estimates the probability that it sample is desired class is: " + str(
        #             torch.exp(logp_desired[0]).item()))
        #     except UnboundLocalError:
        #         print("logp_desired does not exist. The input may be too long!")
